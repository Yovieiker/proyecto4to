  ...
  useEffect(() => {
    const incrementCountClients = () => {
      setCounts((prevCounts) => ({
        ...prevCounts,
        countClients:
          prevCounts.countClients < 10000
            ? prevCounts.countClients + 20
            : prevCounts.countClients,
      }));
    };
    ...
    const incrementCountMarcas = () => {
      setCounts((prevCounts) => ({
        ...prevCounts,
        countMarcas:
          prevCounts.countMarcas < 300
            ? prevCounts.countMarcas + 1
            : prevCounts.countMarcas,
      }));
    };
    ...
    const incrementCountYear = () => {
      setCounts((prevCounts) => ({
        ...prevCounts,
        countYear:
          prevCounts.countYear < 10
            ? prevCounts.countYear + 1
            : prevCounts.countYear,
      }));
    };
    ...
    <>
      <Box sx={{ display: { md: "none" } }}>
        <Box sx={{ background: "#FFED57", textAlign: "center" }}>
          <Box sx={{ py: 10 }}>
            <Typography variant="h4" sx={{ fontWeight: 700, color: "#1A1C3D" }}>
              ¿Cómo funciona{" "}
              <Typography
                variant="h4"
                component="span"
            ...
            {pasosSensebox.map((item) => (
              <Box p={4} key={item.title}>
                ...
                <Typography
                  p={2}
                  variant="h5"
                  sx={{ fontWeight: 700, color: "#1A1C3D" }}
                >
                  {item.title}
                </Typography>
        ...
        >
          ...
          <Typography
            mt={6}
            variant="h3"
            sx={{ fontWeight: 700, color: "#C66CDD" }}
          >
          ...
          >
            <Typography mt={1} sx={{ fontSize: "22px", fontWeight: 600 }}>
              Clientas felices
            </Typography>
            <Box
              mt={3}
              sx={{
                borderBottom: "4px solid #C66CDD",
                width: "80%",
                borderRadius: "10px",
              }}
            ></Box>
          ...
          <Typography mb={10} mt={1} sx={{ fontSize: "22px", fontWeight: 600 }}>
            de experiencia
          </Typography>
          <Button
            component={NavLink}
            ...
            <Box px={8} sx={{ display: "flex" }}>
              {pasosSensebox.map((item) => (
                <Box p={8} key={item.title}>
                  ...
                  <Typography sx={{ color: "#1A1C3D", fontSize: "18px" }}>
                    {item.text}
                  </Typography>
                </Box>
              ))}
